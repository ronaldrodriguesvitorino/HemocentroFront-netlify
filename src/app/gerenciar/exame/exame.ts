...
export class Exame {
  ...
  listar(): void {
    this.http.get<any[]>('/api/exame').subscribe(res => {
  ...
  criar(): void {
    ...
    this.http.post('/api/exame', request).subscribe({
      next: () => {
  ...
  editar(): void {
    const request = { nome: this.nome, descricao: this.descricao };
    this.http.put('/api/exame/' + this.idBusca, request).subscribe({
      next: () => {
  ...
  deletarComConfirmacao(id: number, nome: string): void {
    if (confirm(`Excluir o exame "${nome}"?`)) {
      this.http.delete('/api/exame/' + id).subscribe({
        next: () => this.listar(),
