...
export class Exame {
  ...
  listar(): void {
    this.http.get<any[]>('https://hemocentroback.onrender.com/exame').subscribe(res => {
  ...
  criar(): void {
    ...
    this.http.post('https://hemocentroback.onrender.com/exame', request).subscribe({
      next: () => {
  ...
  editar(): void {
    const request = { nome: this.nome, descricao: this.descricao };
    this.http.put('https://hemocentroback.onrender.com/exame/' + this.idBusca, request).subscribe({
      next: () => {
  ...
  deletarComConfirmacao(id: number, nome: string): void {
    if (confirm(`Excluir o exame "${nome}"?`)) {
      this.http.delete('https://hemocentroback.onrender.com/exame/' + id).subscribe({
        next: () => this.listar(),
